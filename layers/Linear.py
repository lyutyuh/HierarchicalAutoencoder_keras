...
class Linear(Layer):
    def __init__(self, output_size=None, bias=None, bias_start=0.0, activation=None, **kwargs):
        self.output_size = output_size
        self.bias = bias
        self.bias_start = bias_start
        self.activation = activation
        if activation is not None:
            self.activationfunc = activations.get(activation)
        self.matrix = None
    ...
    def build(self, input_shape):
        ...
        if self.bias:
            self._bias = self.add_weight(shape=(self.output_size,),
    ...
    def call(self, inputs):
        ...
        if not self.bias:
            return res
        res = K.bias_add(res, self._bias, data_format='channels_last')
        if self.activation is not None:
            res = self.activationfunc(res)
        return res
